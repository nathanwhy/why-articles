...
> 
> 这是一篇从 [Haskell](https://www.haskell.org/) 移植过来的  [Functors, Applicatives, And Monads In Pictures](http://adit.io/posts/2013-04-17-functors,_applicatives,_and_monads_in_pictures.html)。本文为 Swift 版本。
> 
...
现在当你使用函数处理这个值，根据上下文的不同会得到不同的结果。Functors, Applicatives, Monads，箭头所指的都是基于这个概念。`Optional` 类型定义了两种相关的上下文:
...
这个是就是 `map` 的由来（在 Haskell 是 `fmap`）。`map` 从群众中来，到群众中去。`map` 知道如何使用函数处理数据类型。例如，你想要使用一个函数，将 `.Some(2)` 加 3。使用 `map`:
...
就像黑客帝国中的 Morpheus，`map` 知道要做什么；你开始是 `None`，结束也是 `None`！`map` 是一种禅。现在你可以理解为什么 `Optional` 类型的存在。举个例子，对于没有 `Optional` 类型的语言，比如 Ruby，它们是这么工作的：
...
我们继续深入，applicative 不是开玩笑的。不同于 Haskell，Swift 并不内置 *yet* 语法来处理 applicative。但是这个可以有，我们可以定义一个 `apply` 函数来支持各种类型，从而支持 applicative，applicative 知道如何应用一个封装在数据类型中的函数，转化为相同数据类型的值：
...
如果 `self` 和函数都是 `.Some`，那么函数将被应用于解包的 option，否者，`.None` 被返回。*注意因为 optional 类型是被定义在`Optional<T>`，我们只需要在 `apply` 声明处声明泛型 `U`*
